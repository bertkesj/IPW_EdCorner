 ...
 bootstrap=FALSE,
  ...
  #1. data handling
    if(!is.null(time)){
      ...
      datasorted = sort_by_time(data, id, time)  # utility.R 
    } else {
      datasorted = data
    }

  #3. create copies of data with censoring indicators ----
    datalist = copy_and_censor(datasorted, id, limits) # censoring_api.R
  ...
  #4. estimate weight contribution: censoring at baseline/confounding ----
    bldatalist = grab_first_obs(datalist, id, time) # censoring_api.R
    fudatalist = grab_fu_obs(datalist, id, time) # censoring_api.R
  
  
  #5. estimate weights: censoring during follow-up ----
  lapply(bldatalist, function(x){
    x$deltaIPConfW = estimate_confounding_weights(ipw_conf_formulas, x, id, censvar="__cens") # weight_api.R
  })
  lapply(fudatalist, function(x){
    x$deltaIPCensW = estimate_censoring_weights(ipw_cens_formulas, x, id, censvar="__cens") # weight_api.R
  })
  # aggregate weights
  analysisdatalist = NULL # weight_aggregator.R TODO: check radon example for how this was done
